...
def prog_isendens(sold, snow, unow, dtdx, dthetadt=None):
    ...
    i = nb + np.arange(0, nx)
    snew[i, :] = sold[i, :] - dtdx*(snow[i+1, :]*(unow[i+1,:]+unow[i+2,:])/2 - snow[i-1,:]*(unow[i-1,:]+unow[i,:])/2)
    if idthdt == 1:
        k = np.arange(1,nz-1)
        ii,kk = np.ix_(i,k)
...
def prog_velocity(uold, unow, mtg, dtdx, dthetadt=None):
    ...
    i = nb + np.arange(0, nx+1)  #staggered variables in x-direction have one grid point more
    unew[i,:] = uold[i,:]- unow[i,:]*dtdx*(unow[i+1,:]-unow[i-1,:]) - 2*dtdx*(mtg[i,:]-mtg[i-1,:])
    if idthdt == 1:
        k = np.arange(1, nz-1)
        ii,kk = np.ix_(i,k)
        unew[ii,kk] = unew[ii,kk] - dt/dth*(unow[ii,kk+1]- unow[ii,kk-1])*(dthetadt[ii,kk]+ dthetadt[ii,kk+1]+dthetadt[ii-1,kk]+ dthetadt[ii-1,kk+1])/4 
...
def prog_moisture(unow, qvold, qcold, qrold, qvnow, qcnow, qrnow, dtdx, dthetadt=None):
    ...
    i = nb + np.arange(0, nx)
    qvnew[i, :] = qvold[i,:] - dtdx * (unow[i,:]+ unow[i+1,:])/2 * (qvnow[i+1,:] - qvnow[i-1,:])
    qcnew[i, :] = qcold[i,:] - dtdx * (unow[i,:]+ unow[i+1,:])/2 * (qcnow[i+1,:] - qcnow[i-1,:])
    qrnew[i, :] = qrold[i,:] - dtdx * (unow[i,:]+ unow[i+1,:])/2 * (qrnow[i+1,:] - qrnow[i-1,:])
...
def prog_numdens(unow, ncold, nrold, ncnow, nrnow, dtdx, dthetadt=None):
    ...
    i = nb + np.arange(0, nx)
    ncnew[i, :] = ncold[i,:] - dtdx * (unow[i,:]+ unow[i+1,:])/2 * (ncnow[i+1,:] - ncnow[i-1,:])
    nrnew[i, :] = nrold[i,:] - dtdx * (unow[i,:]+ unow[i+1,:])/2 * (nrnow[i+1,:] - nrnow[i-1,:])
