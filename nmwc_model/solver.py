...
if __name__ == "__main__":
    ...
    k =  nz-nab + np.arange(0, nab)
    tau[k] = diff + (diffabs - diff)*np.sin( np.pi*(k -(nz-nab-1))/(2*nab)  )**2
    ...
    for its in range(1, int(nts + 1)):
        ...
        if imoist == 1 and imicrophys == 1:
            ...
            [lheat,qvnew,qcnew,qrnew,prec,tot_prec] = kessler(snew, qvnew, qcnew, qrnew, prs, exn, zhtnow, th0, prec, tot_prec)
            ...
            #clipping of neg values. not sure this is correct here
            ncnew[ncnew < 0] = 0
            nrnew[nrnew < 0] = 0

            [lheat,qvnew,qcnew,qrnew,tot_prec,prec,ncnew,nrnew]=seifert(unew,th0,prs,snew,qvnew,qcnew,qrnew,exn,zhtold,zhtnow,tot_prec,prec,ncnew,nrnew,dthetadt=dthetadt)
