...
def diag_montgomery(prs, mtg, th0, topo, topofact):
    ...
    mtg[:,0] = ( g*topo.squeeze()*topofact + th0[0]*exn[:,0] ) + dth*exn[:,0]/2

    # Integration loop upwards
    # *** edit here ***

    # use for loop to make it explicitly one after the other
    for k in range(1,nz):
...
def diag_pressure(prs0, prs, snew):
    ...
    # use for loop explicitly one after the other (from top down!)
    for k in range(nz-1, -1, -1): # top most index is nz -> already set through boundary condition -> start at nz-1
